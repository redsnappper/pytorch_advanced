...
class conv2DBatchNormRelu (nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, dilation, bias):
        ...
        self.batchorm = nn.BatchNorm2d(out_channels)
...
class bottleNeckPSP(nn.Module):
    def __init__(self, in_channels, mid_channels, out_channels, stride, dilation):
        ...
        self.cbr_2 = conv2DBatchNormRelu(
            mid_channels, mid_channels, kernel_size=3, stride=stride, padding=0, dilation=dilation, bias=False
        )
...
class PyramidPooling(nn.Module):
    ...
    def forward(self, x):
        ...
        out2 = self.cbr_1(self.avpool_2(1))
        out2 = F.interpolate(out1, size=(
            self.height, self.width), mode="bilinear", align_corners=True)

        out3 = self.cbr_1(self.avpool_3(1))
        out3 = F.interpolate(out1, size=(
            self.height, self.width), mode="bilinear", align_corners=True)

        out4 = self.cbr_1(self.avpool_4(1))
...
class DecodePSPFeature(nn.Module):
        ...
        def forward(self, x):
            ...
            output = F.interpolate(
                x, size=(self.height, self.width), model="bilinear", align_corners=True)
